#!/bin/bash
IP=$(cat /etc/IP)
if [ ! -d /etc/INOVE SSH/userteste ]; then
mkdir /etc/INOVE SSH/userteste
fi
tput setaf 7 ; tput setab 4 ; tput bold ; printf '%30s%s%-15s\n' "Criar usuario teste" ; tput sgr0
echo ""
[ "$(ls -A /etc/INOVE SSH/userteste)" ] && echo -e "\033[1;32mTeste Ativo!\033[1;37m" || echo -e "\033[1;31mNenhum test ativo!\033[0m"
echo ""
for testeson in $(ls /etc/INOVE SSH/userteste |sort |sed 's/.sh//g')
do
echo "$testeson"
done
echo ""
...
echo "$pass" > /etc/INOVE SSH/senha/$nome
echo "$nome $limit" >> /root/usuarios.db
echo "#!/bin/bash
pkill -f "$nome"
userdel --force $nome
grep -v ^$nome[[:space:]] /root/usuarios.db > /tmp/ph ; cat /tmp/ph > /root/usuarios.db
rm /etc/INOVE SSH/senha/$nome > /dev/null 2>&1
rm -rf /etc/INOVE SSH/userteste/$nome.sh
exit" > /etc/INOVE SSH/userteste/$nome.sh
chmod +x /etc/INOVE SSH/userteste/$nome.sh
at -f /etc/INOVE SSH/userteste/$nome.sh now + $u_temp min > /dev/null 2>&1
clear
